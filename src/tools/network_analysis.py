...
def validate_bus_exists(net: pp.pandapowerNet, bus_id: int) -> bool:
    """Check if a bus ID exists in the network."""
    return bus_id in set(net.bus.index.tolist())


def validate_bus_ids_batch(net: pp.pandapowerNet, bus_ids: List[int]) -> Tuple[List[int], List[int]]:
    """
    Validate multiple bus IDs exist in the network.
    
    Returns:
        Tuple of (valid_ids, invalid_ids)
    """
    existing_ids: Set[int] = set(net.bus.index.tolist())
    valid_ids = [bid for bid in bus_ids if bid in existing_ids]
    invalid_ids = [bid for bid in bus_ids if bid not in existing_ids]
    return valid_ids, invalid_ids


class NetworkAnalyzer:
    """Topology analysis and path finding for pandapower networks."""
    ...
    def find_alternative_paths(self, from_bus: int, to_bus: int,
        ...
        """
        # Validate bus IDs exist in network
        if not validate_bus_exists(self.net, from_bus):
            logger.warning(f"Cannot find path: from_bus {from_bus} not in network")
            return []
        if not validate_bus_exists(self.net, to_bus):
            logger.warning(f"Cannot find path: to_bus {to_bus} not in network")
            return []
    ...
    def get_buses_downstream(self, bus_id: int) -> List[int]:
        ...
        """
        # Validate bus ID exists in network
        if not validate_bus_exists(self.net, bus_id):
        ...
        if self.graph is not None:
            ...
            for from_bus, to_bus in bridges:
                ...
                if line_idx is not None:
                    ...
                    if not any(item["line_id"] == line_idx for item in critical_lines):
