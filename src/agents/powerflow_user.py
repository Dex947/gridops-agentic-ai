...
class PowerFlowUserAgent:
    """
    Agent that executes power flow simulations and network modifications.
    Acts as the interface between planning and physical system analysis.
    """
    
    def __init__(self, voltage_limits: tuple = (0.95, 1.05),
                 thermal_limit_percent: float = 100.0):
        """
        Initialize PowerFlow User Agent.
    ...
    def run_baseline_analysis(self, net: pp.pandapowerNet) -> Dict[str, Any]:
        """
        Run baseline power flow analysis on healthy network.
